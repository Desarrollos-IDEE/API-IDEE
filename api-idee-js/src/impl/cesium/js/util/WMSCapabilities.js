...
class GetCapabilities {
        ...
        if (isNullOrEmpty(layerName) || (layer.Name === layerName)) {
          if (!isNullOrEmpty(layer.BoundingBox)) {
            ...
            if (!isNullOrEmpty(bboxSameProj)) {
              ...
              const projSrc = bbox.crs ? bbox.crs.replace('CRS:84', 'EPSG:4326') : bbox.crs;
              const projDest = this.projection_.code;
  ...
  transformExtent(extent) {
    ...
    if (!isNullOrEmpty(extent) && this.capabilities_.version === '1.3.0' && isString(this.capabilitiesProj)) {
      if (this.capabilitiesProj !== 'CRS:84') {
        const proj = proj4(this.capabilitiesProj);
        let axisOrientation = proj.oProj.axis;
        const projectionFound = Object.values(projAPI.getSupportedProjs())
          .find((p) => p.codes.find((code) => code === this.capabilitiesProj));
        if (projectionFound.axisOrientation) {
          axisOrientation = projectionFound.axisOrientation;
        }
