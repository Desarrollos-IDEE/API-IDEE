...
export default class QueryAttributesControl extends IDEE.Control {
  ...
  exportResults() {
    if (this.layer.getFeatures().length > 0) {
      ...
      setTimeout(() => {
        ...
        const selector = 'div.m-api-idee-container div.m-dialog #downloadFormat button';
  ...
  downloadLayer() {
    ...
    const selector = 'div.m-api-idee-container div.m-dialog #downloadFormat select';
   ...
   * @public
   * @function
   * @param {api-idee Layer} layer - clicked layer on select menu
   * @api
   */
  isLayerLoaded(layer) {
    let isLoaded = false;
